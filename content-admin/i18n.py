...
MSG_EN: dict[str, str] = {
    ...
    "guide_command_intro": "Run this on the server, in the directory you installed SMART RAG into:",
    "guide_command_why": "This GUI cannot run it for you: it has no access to Docker or to the server's filesystem, on purpose. A web interface that could start containers would be a much bigger target than one that can only talk to other services over HTTP.",
...
MSG_DE: dict[str, str] = {
    ...
    # --- onboarding guide -------------------------------------------------
    "guide_title": "Erste Schritte",
    "guide_heading": "Erste Schritte",
    ...
    "guide_command_intro": "Führe das auf dem Server aus, in dem Verzeichnis, in das du SMART RAG installiert hast:",
    "guide_command_why": "Diese Oberfläche kann das nicht für dich tun: Sie hat bewusst keinen Zugriff auf Docker und auf das Dateisystem des Servers. Eine Weboberfläche, die Container starten könnte, wäre ein deutlich größeres Angriffsziel als eine, die nur per HTTP mit anderen Diensten spricht.",
