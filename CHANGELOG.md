...
- **The concept-map proposal came back as "Nothing was pasted."** Pressed on
  the test installation, it ran for six seconds and reported that nothing had
  been pasted — by an operator who had pressed a button.

  `LLM_MODEL_STRONG` there is a reasoning model, and OpenAI's
  `max_completion_tokens` counts reasoning tokens. The ceiling was 1024,
  shared with the keyword suggestions it was sized for; the model spent all of
  it thinking and returned an empty `content` with `finish_reason: "length"` —
  a successful call, as far as the API is concerned. The empty string then
  ...
  hit, and that nothing was written. Three of those four could not report it
  at all before: Google raised a shape error, Cohere the same, and Anthropic
  said "no text content block", which reads like a protocol fault rather than
  an exhausted budget.

  While fixing it: Cohere was the one provider still sending the fixed 1024
  rather than the budget it was given. `tests/test_llm_budget.py` checks all
  six provider shapes, and eight mutations of the fix turn it red.

- **`credentials.txt` was never actually ignored, and it holds the initial
  passwords.** The rule had been in `.gitignore` since the beginning, written
  as `credentials.txt          # generated by bootstrap.sh …`. In `.gitignore`
  a `#` starts a comment **only at the beginning of a line**; anywhere else it
  is an ordinary character. The pattern was therefore the entire sentence, and
  matched no file that could ever exist. Found on the test installation, where
  the file sat untracked in the working tree — one `git add -A` from being
  published — carrying the initial passwords for Postgres, Neo4j, ClickHouse,
  Flowise and the Content Admin.

  Nothing leaked: `git log --all -- credentials.txt` is empty, and no `.env`,
  key, certificate or credential file appears in any commit that has ever
  existed. The comment now stands on its own line, `*.crt` and `*.csr` are
  covered too — Tailscale writes a certificate into the repository directory —
  and `tests/test_gitignore.sh` checks every pattern line for the same shape,
  checks each secret file this project's own scripts create by name against
  the real `git check-ignore`, and checks that none of them is tracked or ever
  was.
