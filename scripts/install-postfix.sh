...
# ─── Arg parsing ─────────────────────────────────────────────────────────────
# --test-only: skip install/config entirely, just send a test email against
# whatever relay is already configured. Used by scripts/admin.sh's Mail menu
# item so it doesn't have to duplicate the queue-ID-polling logic below.
TEST_ONLY=0
while (( $# > 0 )); do
    case "$1" in
